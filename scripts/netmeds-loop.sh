...
configured_cap="${AUSHADHI_NETMEDS_CAP:-20000}"
case "$configured_cap" in ''|*[!0-9]*|0) configured_cap=$MAX_CAP ;; esac
if [ "$configured_cap" -gt "$MAX_CAP" ]; then
  echo "$(date -u '+%FT%TZ') netmeds: requested cap $configured_cap exceeds policy; clamped to $MAX_CAP"
  configured_cap=$MAX_CAP
fi
export AUSHADHI_NETMEDS_CAP="$configured_cap"
export AUSHADHI_DISTINCT_EXIT_CODES=1
readonly HOLD_MARKER="${AUSHADHI_RAW_ROOT:-data/raw}/netmeds/operator-hold"

persist_operator_hold() {
  local reason="$1"
  node --input-type=module - "$HOLD_MARKER" "netmeds" "$reason" <<'NODE'
...
enter_operator_hold() {
  ...
  if ! persist_operator_hold "$reason"; then
    echo "$(date -u '+%FT%TZ') netmeds FATAL: operator-hold publication failed; refusing all further source requests"
    exec sleep infinity
  fi
  echo "$(date -u '+%FT%TZ') netmeds HOLD: $reason; marker=$HOLD_MARKER"
  echo "netmeds HOLD: clearance requires: stop the service, investigate, explicitly delete the marker, then start the service; this loop never clears it"
  exec sleep infinity
}
...
sleep_until_utc_reset() {
  ...
  echo "$(date -u '+%FT%TZ') netmeds: cap reached — reset wait ${delay}s until after 00:00 UTC"
  sleep "$delay"
}
...
while true; do
  ...
  case "$rc" in
    2)
      ...
      ;;
    3)
      echo "$(date -u '+%FT%TZ') netmeds HOLD: blocked/robots; automatic retries disabled pending human clearance"
      enter_operator_hold "blocked/robots"
      ;;
    4)
      echo "$(date -u '+%FT%TZ') netmeds ERROR: discovery/parser anomaly — sleeping 1h before fail-closed retry"
      sleep 3600
      ;;
    5)
      echo "$(date -u '+%FT%TZ') netmeds: NO_WORK: scheduled idle 6h"
      sleep 21600
      ;;
    0)
      echo "$(date -u '+%FT%TZ') netmeds: crawl complete — scheduled idle 6h (re-check for new products)"
      sleep 21600
      ;;
    *)
