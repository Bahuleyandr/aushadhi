...
export AUSHADHI_APOLLO_CAP="${AUSHADHI_APOLLO_CAP:-10000}"
echo "$(date -u '+%FT%TZ') apollo-loop start (cap=${AUSHADHI_APOLLO_CAP}/day, spacing=2.5s)"

while true; do
  node src/cli/apollo.mjs; rc=$?
  if [ "$rc" -eq 2 ]; then
    echo "$(date -u '+%FT%TZ') apollo: daily cap / block — sleeping 1h for reset"
    sleep 3600
  elif [ "$rc" -eq 0 ]; then
    echo "$(date -u '+%FT%TZ') apollo: crawl complete — idle 6h (re-check for new products)"
    sleep 21600
  else
